# -*- coding: utf-8 -*-
"""
Display and control a Focusmate countdown.
...
Configuration parameters:
    ...
    sound_focusmate_end: focusmate end sound (file path) (requires pyglet
        or pygame) (default None)
    sound_focusmate_start: focusmate start sound (file path) (requires pyglet
        od pygame) (default None)
    timer_focusmate: focusmate time (seconds) (default 3000)
...
Color options:
    color_bad: Focusmate not running
    color_good: Focusmate active
...
class Py3status:
    ...
    format_separator = u":"
    num_progress_bars = 5
    sound_focusmate_end = None
    sound_focusmate_start = None
    timer_focusmate = 50 * 60
    ...
    def _init(self):
        self._active = True
        self._running = False
        self._time_left = self.timer_focusmate
        self._section_time = self.timer_focusmate
        self._prefix = 'Focusmate'
        self._timer = None
        self._end_time = None
        self._player = Player()
        self._format = 'Focusmate {{time}}'
        self._alert = False
    ...
    def _advance(self, user_action=False):
        ...
        if self._active:
            self._active = False
            if not user_action:
                self._play_sound(self.sound_focusmate_end)
        self._time_left = self.timer_focusmate
        self._section_time = self.timer_focusmate
    ...
    def on_click(self, event):
        ...
        if event['button'] == 1:
            if self._running:
                ...
                self._timer.start()
                if self._active:
                    self._play_sound(self.sound_focusmate_start)
    ...
    def focusmate(self):
        """
        Focusmate response handling and countdown
        """
    ...
    def _play_sound(self, sound_fname):
        ...
        if not self._player.available:
            self.py3.log("focusmate module: the pyglet or pygame "
